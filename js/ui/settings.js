...
export async function renderSettings(ctx) {
  ...
  const rateRows = CATEGORIES.map((cat) => {
    ...
    const mk = (key) => {
      const i = el("input", { class: "input", type: "number", inputmode: "numeric", value: r[key] });
      i.addEventListener("blur", () => { s.baseRates[cat] = { ...s.baseRates[cat], [key]: parseInt(i.value, 10) || 0 }; persist(); });
      return i;
    };
...
function group(title, children) {
  return el("div", { class: "set-group" }, [el("h2", { text: title }), el("div", { class: "card stack" }, children)]);
}
...
function aiGroup(ctx, s, persist) {
  ...
  testBtn.addEventListener("click", async () => {
    ...
    try {
      const { testKey } = await import("../engine/ai.js");
      s.ai.apiKey = key.value.trim();
      persist();
      const ok = await testKey(s);
      toast(ok ? "Key works" : "Key responded oddly", ok ? "success" : "error");
    } catch (e) {
...
async function dataGroup(ctx, s) {
  ...
  const loadExamplesBtn = el("button", { class: "btn btn--secondary btn--full" }, [icon("upload", "btn__icon"), "Load example leads"]);
  loadExamplesBtn.addEventListener("click", async () => {
    const added = await loadExampleLeads(ctx.settings);
    toast(added ? `Loaded ${added} example leads` : "Example leads are already loaded", added ? "success" : "");
    ctx.reload();
  });

  const removeExamplesBtn = el("button", { class: "btn btn--ghost btn--full" }, [icon("trash", "btn__icon"), "Remove example leads"]);
  removeExamplesBtn.addEventListener("click", async () => {
    const removed = await removeExampleLeads();
    toast(removed ? `Removed ${removed} example leads` : "No example leads found", removed ? "success" : "");
    ctx.reload();
  });
  ...
  clearBtn.addEventListener("click", async () => {
    ...
    store.writeSentinel(0, 0, s.lastBackupAt);
  ...
  return el("div", { class: "set-group" }, [
    ...
    el("div", { class: "card stack" }, [
      ...
      el("div", { class: "hint", text: exampleCount ? `${exampleCount} example lead${exampleCount === 1 ? "" : "s"} currently in this app.` : "No example leads loaded." }),
      exportBtn,
      importBtn,
      fileInput,
      loadExamplesBtn,
      exampleCount ? removeExamplesBtn : null,
      clearBtn,
    ]),
  ]);
}
...
async function onImport(ctx, fileInput) {
  ...
  let text;
  try { text = await file.text(); } catch { toast("Couldn't read the file.", "error"); return; }
  let obj;
  try { obj = JSON.parse(text); } catch { toast("That isn't valid JSON.", "error"); return; }
  ...
  try {
    ...
    if (res.data.settings) {
      const merged = { ...ctx.settings, ...res.data.settings, ai: ctx.settings.ai };
...
async function onRescore(ctx) {
  ...
  for (const lead of leads) {
    try {
      await store.putLead(analyzeAll(lead, ctx.settings));
      n++;
    } catch {}
  }
