...
const SEED_FLAG = `lor:${CHANNEL}:seeded`;
...
export async function seedIfEmpty(settings) {
  try {
    if (localStorage.getItem(SEED_FLAG)) return false;
    const existing = await store.getAllLeads();
    if (existing.length > 0) { localStorage.setItem(SEED_FLAG, "1"); return false; }
    const now = Date.now();
    let i = 0;
    for (const s of SAMPLES) {
      const lead = newLead({ ...s, createdAt: new Date(now - (i * 9 + 1) * 3600000).toISOString() });
    ...
    return true;
  } catch (e) { store.logError("seed", e.message, e.stack); return false; }
}
